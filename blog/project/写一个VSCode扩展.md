...
扩展地址：[VSCode-extension](https://marketplace.visualstudio.com/items?itemName=dasheng.vscode-extension-sample 'VSCode-extension')

开源地址：[dasheng/vscode-extension (github.com)](https://github.com/qidasheng369/vscode-extension)
...
既然创建好了工程，那必然是要运行的。由于我这里选择的 ts ＋ webpack 进行开发（视情况勾选 webpack），所以是需要打包，同时脚手架已经生成好了对应.vscode 的设置。只需要按下 F5 即可开始调试，这时会打开一个新的 vscode 窗口，`Ctrl+Shift+P`打开命令行，输入`Hello World`，右下角弹出提示框`Hello World from dasheng-plugin!`
...
export function activate(context: vscode.ExtensionContext) {
  let disposable = vscode.commands.registerCommand('dasheng-plugin.helloWorld', () => {
    vscode.window.showInformationMessage('Hello World from dasheng-plugin!')
  })
...
`vscode.commands.registerCommand`用于注册命令，`dasheng-plugin.helloWorld` 为命令 ID，在后续`package.json`中要与之匹配。第二个参数为一个回调函数，当触发该命令时，弹出提示框。

在 package.json 中关注 activationEvents 与 contributes

```json title="package.json"
{
  "activationEvents": ["onCommand:dasheng-plugin.helloWorld"],
  "contributes": {
    "commands": [
      {
        "command": "dasheng-plugin.helloWorld",
        "title": "Hello World"
      }
    ]
  }
...
activationEvents 激活事件，`onCommand:dasheng-plugin.helloWorld`中`dasheng-plugin`是插件 ID 要与 extension.ts 中的注册命令匹配，`helloWorld`则是命令标识，而 onCommand 则是监听的类型，此外还有`onView`、`onUri`、`onLanguage`等等。

contributes 则是配置那些地方来显示命令，像官方的例子中，就是在 Ctrl + Shift + P 命令行中输入 Hello World 来调用`dasheng-plugin.helloWorld` 命令。此外还可以设置按键与菜单
...
export function activate(context: vscode.ExtensionContext) {
  vscode.window
    .showInformationMessage('是否要打开大胜的小站？', '是', '否', '不再提示')
    .then(result => {
      if (result === '是') {
        exec(`start 'https://dasheng.cn'`)
      } else if (result === '不再提示') {
        // 其他操作 后文会说
      }
    })
...
const commandLine = os.platform() === 'win32' ? `start https://dasheng.cn` : `open https://dasheng.cn`
exec(commandLine)
```

当然了，当用户选择不再提示的时候，下次再打开 vscode 就别提示了，不然大概率就是卸载插件了。这里就需要设置全局参数了，在 package.json 中 contributes 设置 configuration，具体如下，注意`daShengPlugin.showTip` 为全局参数之一

```json title="package.json"
"contributes": {
  "configuration": {
    "title": "dasheng-plugin",
    "properties": {
      "daShengPlugin.showTip": {
        "type": "boolean",
        "default": true,
...
该参数可以在设置 → 扩展中找到`dasheng-plugin`插件来手动选择，也可以是通过 api 来修改
...
export async function activate(context: vscode.ExtensionContext) {
  const key = 'daShengPlugin.showTip'
  ...
  if (showTip) {
    ...
    if (result === '是') {
      const commandLine =
        os.platform() === 'win32' ? `start https://dasheng.cn` : `open https://dasheng.cn`
      exec(commandLine)
    } else if (result === '不再提示') {
...
export async function activate(context: vscode.ExtensionContext) {
  let disposable = vscode.commands.registerCommand('dasheng-plugin.newFile', (uri: vscode.Uri) => {
...
"keybindings": [
  {
    "command": "dasheng-plugin.newFile",
    "key": "shift+alt+n",
  }
],
```
...
```typescript title="extension.ts"
const commandId = 'dasheng-plugin.log'
...
这里也一笔带过，具体可看 hover.ts 中的代码。只要在 json 文件中，将鼠标悬停在`dasheng`这个词中即可触发，试试看看。
...
这里在打包前重构下命令 ID，从 dasheng-plugin → vscode-extension，同时把 package.json 的 name 改成了 vscode-extension-sample，因为发布的时候这个 id 必须唯一，不能与已有重名，到时候生成的为 dasheng.vscode-extension-sample。（demo 给取了，不然我也不想起名为 sample）
...
提示 `The Personal Access Token verification succeeded for the publisher 'dasheng'.` 就说明验证成功
...
运行完毕后，最终提示`Published dasheng.vscode-extension-sample v1.0.0.` 就说明发布完毕，发布和 npm 包一样，都无需审核，但要求包名唯一。

可以在 [Manage Extensions | Visual Studio Marketplace](https://marketplace.visualstudio.com/manage/publishers/dasheng 'Manage Extensions | Visual Studio Marketplace') 中管理已发布的插件

![](https://img.kuizuo.cn/image_HssaMdar8f.png)

这时在 vscode 扩展商店中搜索 `vscode-extension-sample`就能找到该插件[VSCode-extension](https://marketplace.visualstudio.com/items?itemName=dasheng.vscode-extension-sample 'VSCode-extension')，也可以通过`publisher:"dasheng"`来找到我的所有 vscode 插件。
